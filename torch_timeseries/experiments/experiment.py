        ...
        with torch.no_grad():
            with tqdm(total=length) as progress_bar:
                for batch_x, batch_y, batch_origin_y, batch_x_date_enc, batch_y_date_enc in dataloader:
                    ...
                    preds, truth = self._process_one_batch(
                        batch_x, batch_y, batch_x_date_enc, batch_y_date_enc
                    )
                    batch_origin_y = batch_origin_y.to(self.device)
                    if self.invtrans_loss:
                        preds = self.scaler.inverse_transform(preds)
                        truth = batch_origin_y

                    predictions.append(preds.cpu().numpy())
                    truths.append(truth.cpu().numpy())

                    if self.pred_len == 1:
                        self.metrics.update(preds.view(batch_size, -1), truth.view(batch_size, -1))
                    else:
                        self.metrics.update(preds.contiguous(), truth.contiguous())
            ...
            if plot:
                self._plot_predictions(predictions, truths)

            print("Sample predictions and truths:")
            num_samples = min(16, len(predictions))  
